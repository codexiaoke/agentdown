...
import type {
  ...
  SpringAiRunSession,
  SpringAiSessionState,
  SpringAiToolPayload
} from './types';

/**
 * 创建一份全新的 Spring AI 会话状态容器。
 */
export function createSpringAiSessionState(): SpringAiSessionState {
  return {
    sessions: new Map<string, SpringAiRunSession>(),
 ...
 * 重置 Spring AI 会话状态容器。
 */
export function resetSpringAiSessionState(state: SpringAiSessionState): void {
 ...
 */
function syncStreamSegmentIds(session: SpringAiRunSession): void {
 ...
 */
function resolveSessionMessageScope(session: SpringAiRunSession) {
  return {
...
/**
 * 在一次内容分段完成后切换到下一段内容 id。
 */
function advanceStreamSegment(session: SpringAiRunSession): void {
  session.streamSegmentIndex += 1;
  syncStreamSegmentIds(session);
}

/**
 * 把当前 response 范围切换到新 message 上。
 */
export function syncResponseScope(
  session: SpringAiRunSession,
  packet: SpringAiEvent,
  context: ProtocolContext,
  options: SpringAiProtocolOptions
): void {
  const nextGroupId = resolveGroupId(session.runId, packet, context, options);
  const nextConversationId = resolveConversationId(session.runId, packet, context, options);
  const nextTurnId = resolveTurnId(session.runId, packet, context, options);
  const nextMessageId = resolveMessageId(session.runId, packet, context, options);

  session.groupId = nextGroupId;
  ...
  session.streamBaseId = resolveStreamId(session.runId, packet, context, options);
  session.blockBaseId = resolveBlockId(session.runId, packet, context, options);
  session.streamSegmentIndex = 0;
...
export function ensureRunSession(
  ...
  const created: SpringAiRunSession = {
    runId,
 ...
 */
function createFallbackToolId(session: SpringAiRunSession): string {
...
export function ensurePendingTool(
  ...
  const created: SpringAiPendingTool = {
