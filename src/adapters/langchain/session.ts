...
import type {
  LangChainEvent,
  LangChainPendingTool,
  LangChainProtocolOptions,
  LangChainRunSession,
  LangChainSessionState,
  LangChainToolPayload
} from './types';

/**
 * 创建一份全新的 LangChain 会话状态容器。
 */
export function createLangChainSessionState(): LangChainSessionState {
  return {
    sessions: new Map<string, LangChainRunSession>(),
 ...
 * 重置 LangChain 会话状态容器。
 */
export function resetLangChainSessionState(state: LangChainSessionState): void {
 ...
 */
function syncStreamSegmentIds(session: LangChainRunSession): void {
 ...
 */
function resolveSessionMessageScope(session: LangChainRunSession) {
  return {
 ...
 */
function advanceStreamSegment(session: LangChainRunSession): void {
...
export function ensureRunSession(
  ...
  const created: LangChainRunSession = {
    runId,
...
): LangChainPendingTool | undefined {
 ...
 */
function createFallbackToolId(session: LangChainRunSession): string {
...
export function ensurePendingTool(
  ...
  const created: LangChainPendingTool = {
