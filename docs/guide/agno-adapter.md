...
```ts
import {
  // adapter 会把 Agno 官方协议、markdown assembler 和 surface 默认值收起来。
  createAgnoAdapter,
  // Agno 场景最常见的后端请求 helper。
  createAgnoSseTransport,
  // 页面层直接拿响应式 session，不需要再手动拼 bridge 状态。
  useAdapterSession,
...
// adapter 统一承载协议、helper 和 surface 约定。
...
// 页面层直接得到 runtime、surface 和 connect / restart / error / status。
...
- 不需要自己 `defineAgnoPreset()`
- 不需要手动 `preset.createSession()`
- 不需要再额外包一层 `useBridgeTransport()`
